...
const MultipleCorrectQuestion = ({ question, setQuestions, index }: any) => {
    ...
    if (question.options.length === 0) {
      setQuestions((prevQuestions:any) => {
        ...
        updatedQuestions[index] = {
          ...
          correctAnswer: ["Default Option"], // Initialize correctAnswer as an array
        };
      ...
      setQuestions((prevQuestions:any) => {
        ...
        updatedQuestions[index] = {
          ...
          correctAnswer: [updatedQuestions[index].options[0].text], // Ensure at least one correct answer
        };
        return updatedQuestions;
      });
    }
  ...
  const updateCorrectAnswerArray = (
    updatedOptions: { text: string; isCorrect: boolean }[]
  ) => {
    return updatedOptions.filter((opt) => opt.isCorrect).map((opt) => opt.text); // Collect all correct option texts
  };
  ...
  const handleOptionChange = (optionIndex: number, value: string) => {
    setQuestions((prevQuestions:any) => {
      ...
      updatedQuestions[index] = {
        ...
        correctAnswer: updateCorrectAnswerArray(
          updatedQuestions[index].options.map((opt:any, idx:any) =>
            idx === optionIndex ? { ...opt, text: value } : opt
          )
        ),
      };
  ...
  const handleRemoveOption = (optionIndex: number) => {
    setQuestions((prevQuestions:any) => {
      ...
      updatedQuestions[index] = {
        ...
        correctAnswer: updateCorrectAnswerArray(updatedOptions), // Update correct answers
      };
      return updatedQuestions;
    });
  };
  ...
  return (
    <div className="mt-4">
      ...
      <div className="mt-4">
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
          Options (Select multiple correct options)
        </label>
